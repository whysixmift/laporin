...
pub async fn start_generation_handler(
    ...
    if JobRepo::has_active_generation_job(&state.db, id).await? {
        return Err(AppError::JobAlreadyActive);
    }

    // 3. Enqueue generation job
    let job = JobRepo::enqueue_generation_job(&state.db, id).await?;

    // 4. Update report status to generating
    ReportRepo::update_status(&state.db, id, "generating").await?;

    Ok((StatusCode::ACCEPTED, Json(job)))
}

pub async fn get_generation_status_handler(
    State(state): State<AppState>,
    auth: AuthenticatedUser,
    Path(id): Path<Uuid>,
    ...
    let job = JobRepo::get_latest_generation_job(&state.db, id)
        .await?
        .ok_or_else(|| AppError::NotFound("No generation job found for this report".into()))?;
